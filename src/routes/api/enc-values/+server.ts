...
import { BEARER_TOKEN, PRIVATE_KEY } from '$env/static/private';
...
export async function GET({ request }) {
	const message = { BEARER_TOKEN: BEARER_TOKEN };
	const signature = EthCrypto.sign(PRIVATE_KEY, EthCrypto.hash.keccak256(JSON.stringify(message)));
